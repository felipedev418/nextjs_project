...
const Navbar = () => {
  ...
  const [isJackpotPage, setIsJackpotPage] = useState(true);
    ...
    <>
      <div className="flex fixed items-center justify-between pl-6 pr-96 pring py-[10px] z-50  w-[100%] mt-5 md:mt-0">
        ...
        <div className="flex items-center gap-4 z-50">
          ...
          )}
          {
            <Button
              ...
              variant={"ghost"}
              className="!border !border-[#292828] !bg-[#21201F] w-10 h-10 !rounded-xl !p-3  xl:hidden"
            >
        ...
        <div
          ...
          } fixed inset-0 h-[100vh]  bg-black bg-opacity-50 backdrop-blur-sm xl:hidden z-40`}
        ></div>
        <div
          className={`${
            mobileNavOpen ? "scale-100 origin-center" : "scale-0"
          } xl:hidden z-40  fixed top-24 w-[calc(100%-52px)] mx-auto h-[428px] right-0 left-0 bottom-0 bg-[#21201F] py-[19.5px] px-[16.75px] rounded-[32px] flex flex-col gap-[27px]  transition-all`}
        >
          <div className=" xl:hidden flex w-full flex-col gap-3">
            {navbtn.map((nav, idx) => (
              <Link
                key={idx}
                href={
              ...
              >
                ...
                <h2 className="text-sm font-medium leading-4 text-[#686868]">
                  {idx === 0 ? (isJackpot ? "Jackpot" : "Coinflip") : nav.name}
                </h2>
              </Link>
            ))}
          </div>
        ...
        <Button
          ...
          className=" fixed right-[27px] z-50 top-[730px] flex items-center justify-center xl:hidden w-10 h-10 rounded-[13px] bg-gradient-to-t p-[10px] from-[#EC1B24] to-[#FF323B]"
        >
