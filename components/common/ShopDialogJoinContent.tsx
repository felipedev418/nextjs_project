...
const ShopDialogJoinContent = () => {
    ...
    const handleSelection = (idx: number) => {
        if (selected.includes(idx)) {
            ...
            selected.length!==6 && setSelected([...selected, idx]);
        }
    };
    return (
        <DialogContent className="w-fit border-none flex flex-col lg:flex-row">
            <VisuallyHidden>
                <DialogTitle>ShopDialogJoinContent</DialogTitle>
            </VisuallyHidden>
            ...
            <div className="  flex flex-col rounded-xl bg-[#21201F] border border-[#292828] w-full sm:w-[330px] gap-[19px] h-fit">
                <div className="flex items-center justify-center py-[14px] border-b border-b-[#272625]">
                    <h2 className="leading-[19.2px] font-fredoka">
                        Join Coinfilp
                    </h2>
                </div>
                <h2 className="text-xl text-center leading-[21.6px] text-[#ffffff]">
                    Available:{40 - selected.length}
                </h2>
                <h2 className="text-lg text-center leading-[21.6px] text-[#9C9C9C]">
        ...
        <div className="flex flex-col items-center gap-3">
          <h2 className="text-lg leading-[21.6px] ">Join Coinfilp</h2>
          <h2 className="text-xl leading-[21.6px] text-[#9C9C9C]">Available:{40-selected.length}</h2>
          <h2 className="text-lg leading-[21.6px] text-[#ffffff]">Selected:{selected.length}</h2>
        </div>
        <Button variant={"secondary"}>Join Coinfilp</Button>
      </div> */}
        </DialogContent>
    );
};

export default ShopDialogJoinContent;
